...
%The relative proximity of the LDO equations' solutions to those generated with
%the QR quadrature set prompts the exploration of the LDO equations' solutions 
%as input for Monte Carlo variance reduction parameter generation.
% Do we talk about that in this paper? It seems a little bit out of nowhere. I've shifted this a little bit; see what you think.
...
% I'm taking a lot of the references to Denovo out as this is all generic.
...
% There's only one sn approximation, it's not specific to any piece of software
...
% You may want to add something about the scattering moments. That term still isn't defined. 
...
differences. From the derivation in Reference \cite{ahrens}, the equations, without energy discretization, are written
as
...
of equations with respect to what scattering data is needed and how scattering is handled from the perspective of 
implementation.
...
\end{equation}
% it makes sense to use denovo here b/c the normalization is an implementation choice :)
...
than the \sn\ order.
% Really good description of problems and parameters. I think this is strong on the reproducibilty potential.

%---------------------------------------------------------------------------%%
\section{Results}
\label{sec:results}

%Intro paragraph, or at least a sentence. 
...
flux to one of the flux from one of the standard quadrature set types. The relative
...
% Again, this sort of feels like it's out of nowhere. I'd consider re-framing, potentially in a way that is similar to what I did in the intro. 
...
reference. We see that, on average, the LDO flux matches
the QR flux better than it matches the other quadrature types. However, in this case, the LDFE flux solution on average 
matches the QR flux solution better than any other quadrature type does, including the LDO flux solution (5\% difference versus 8.4\% difference).
% I think these two sentences make more sense now. They were sort of contradictory how they were written before. 
...
% Something I just notices: you can probably take "forward" out everywhere. I know it's a holdover from your dissertation, it's just that there isn't any other type of flux discussed. 
...
the flux solutions, seen in Figure \ref{cargo-fwd-slices}, display ray effects as a result of the streaming paths
created by the material variation of the pallets inside of the shipping
container.
% Double check that all figures and tables are referred to.

% I'm also not sure why the LDO flux is "representative". I think that's also a dissertation holdover?
...
% For the photon problems, may want to add photon to remind folks. Not sure that it really matters though. 
\begin{table}[!hbt]
\centering

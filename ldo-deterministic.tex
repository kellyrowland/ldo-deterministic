...
flux moments than the traditional discrete ordinates equations. 
% good!
We again note
...
all other listed run times. One of the main causes of the time difference is likely the octahedral
...
an equal number of angles among all octants, so asymmetric quadratures do not benefit as much from the angle pipelining of KBA. We also see that the simulation
using the LDO quadrature set has a greater memory requirement than those with
other quadrature types. Recalling Section \ref{sec:op}, 
% there's a period after the section ref that shouldn't be there
this is to be
...
% It might be quite helpful to say something here to remind us why we're bothering to consider LDO if it's more expensive and uses more memory.
...
An important point of discussion is simulation results as a
function of angular mesh refinement. We dive into this examination for the
DLVN benchmark because the demonstrated errors are those relative to actual
measurements determined experimentally rather than only other simulations. In the interest of brevity, we have
...
\end{figure}
% can you make it so it's easier to tell what numbers are on the y axis?
...
mesh refinement.
% maybe specifically call out that LDO gives the lowest error in fig 8?
...
% I was wondering this before, how can we tell if all the slowness is from pipelining? Is the larger memory also causing slowdown b/c of data access / communication or anything like that?
...
For the test cases presented, we have compared the scalar flux solutions from solving the LDO equations to those from solving
...
its operator sizes scale with the number of angles. As a result, it is not
surprising that the simulations with the LDO quadrature set have an increased
memory requirement.
% AGain, we need some wording here about why in the world anyone would use LDO. Remind us about the benefit of it and what makes it interesting. 
